...
namespace Itenso.Sys
{
    public static class StringTool
    {
        ...
        ///         <item>
        ///             escape is followed by 'x': the next two characters are interpreted
        ///             as a hex code of the character to be inserted
        ///         </item>
        ...
        /// <param name="escape">
        ///     an escaping character to use both within and outside
        ///     of quoted sections, e.g. '\\'
        /// </param>
        /// <param name="includeEmptyUnquotedSections">
        ///     whether to return zero-length sections
        ///     outside of quotations or not. empty sections adjacent to a quoted section are
        ///     never returned.
        /// </param>
        /// <param name="separator">
        ///     the separator character(s) which will be used to
        ///     split the string outside of quoted sections
        /// </param>
        /// <returns>
        ///     the array of sections into which the string has been split up.
                    ...
                    if (i < length - 1)
                    {
                        ...
                        switch (c)
                        {
                            ...
                            case 'x':
                                if (i < length - 2)
                                {
                                    var upperHexNibble = GetHexValue(toSplit[i + 1]) * 16;
                                    var lowerHexNibble = GetHexValue(toSplit[i + 2]);
                                    var hexChar = (char) (upperHexNibble + lowerHexNibble);
